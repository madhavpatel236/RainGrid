...
const RainGrid = () => {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setGridColors((prevGrid) => {
        const newGrid = prevGrid.map((row) => [...row]); // Clone  grid


        // Process each column independently
        for (let col = 0; col < cols; col++) {
          for (let row = rows; row >= 0; row--) { // Include extra row (rows + 1)
            if (newGrid[row][col]) {
              ...
              if (row < rows && !newGrid[row + 1][col]) {
                ...
                // Remove the color from the last row after it drops
                newGrid[row][col] = null;
              }
            }
          }
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden flex justify-center items-center">
      ...
      {/* Main Grid with Falling Rain Drops */}
